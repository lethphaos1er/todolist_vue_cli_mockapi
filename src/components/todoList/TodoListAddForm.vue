<script setup>
import { ref } from 'vue';
  // ref("") est un conteneur de donnée
  const newContent= ref("");
  // utile pour faire le emits dans const onSubmitAddForm
  const emits = defineEmits(["onSubmitAddForm"]);

  // C'est le onSubmitAddForm de <form class="flex gap-2" aria-label="Add a new task" @submit.prevent="onSubmitAddForm()">
  //utile pour allèger le template.

  const onSubmitAddForm=()=>{
    emits('onSubmitAddForm', newContent);
    newContent.value=""; 
  };
</script>
<template>
